...
| `accepted_terms` | boolean/string/int | да | Должно быть истинным значением: `true`, `1`, `"1"`, `"true"`, `"on"` или `"yes"`. |
| `accepted_personal_data` | boolean/string/int | да | Должно быть истинным значением. Подтверждает согласие на обработку персональных данных. |
| `accepted_marketing` | boolean/string/int | нет | Если истинное значение — дополнительно сохраняется согласие на информационные письма. |

**Успех:** `201`, возвращает сообщение и созданного пользователя с `accepted_terms`, `accepted_personal_data`, `accepted_marketing`, версиями документов, датами принятия, `requires_email_verification: true` и сроком действия письма подтверждения.  
**Побочный эффект:** после создания пользователя пишет записи о согласии в `user_consents`: обязательные `terms` и `personal_data`, а при `accepted_marketing = true` ещё `marketing_emails`; создаёт запись в `email_verification_tokens` и отправляет письмо через `notify@native-places.ru`.  
**Перед обновлением `register.php`:** сначала выполнить миграции `docs/sql/2026-07-05-create-user-consents.sql` и `docs/sql/2026-07-08-create-email-verification-tokens.sql` на БД хоста. Миграции намеренно не создают `FOREIGN KEY`, чтобы не падать на хостингах с отличающимся типом `users.id`; связь контролируется индексом `user_id` и кодом регистрации.  
**Ошибки:** `400` некорректный JSON, `422` валидация, `409` email уже существует, `500` ошибка регистрации или отправки письма.

### `GET|POST /auth/verify-email.php`

Подтверждает email по токену из письма. Токен можно передать в query string `?token=...` или JSON-теле `{ "token": "..." }`.

**Успех:** заполняет `users.email_verified_at`, помечает токен `used_at` и возвращает `email_verified: true`.  
**Ошибки:** `404` токен не найден, `410` срок действия истёк, `422` токен уже использован, `403` пользователь заблокирован, `409` email пользователя изменился.
...
**Успех:** если email не подтверждён, закрывает старые активные токены, создаёт новый токен на 24 часа и отправляет письмо. Если email уже подтверждён, возвращает сообщение без отправки письма.
